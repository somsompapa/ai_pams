...
# 매일 자동화: 서버 crontab 등록
crontab -e
# 아래 줄들을 추가:

# 1) 장중 시세 자동수집 — 국내(KRX)·미국 장 시간에 30분 간격으로 fetch + signals
#    (fetch/signals는 멱등적이라 여러 번 실행해도 안전하고, 새로 발동한 신호만 알림)
# 국내: 평일 09:00~15:30 KST
0,30 9-15 * * 1-5 docker exec pams sh -c 'python -m pams.interfaces.cli fetch && python -m pams.interfaces.cli signals'
# 미국(서머타임 포함 여유있게): 평일 22:00~23:59 KST
0,30 22-23 * * 1-5 docker exec pams sh -c 'python -m pams.interfaces.cli fetch && python -m pams.interfaces.cli signals'
# 미국 자정 이후분: 화~토 00:00~06:00 KST (월~금 저녁 장의 연장)
0,30 0-6 * * 2-6 docker exec pams sh -c 'python -m pams.interfaces.cli fetch && python -m pams.interfaces.cli signals'

# 2) 마감 후 스냅샷 적재 + 규칙 알림 (하루 1회, 평일 18시)
...
장중 시간대는 서머타임(DST)까지 정확히 맞추려 하지 않고 넉넉하게 잡았다 —
장 마감 후 호출은 fetch가 0건을 수집할 뿐 문제가 되지 않는다.
