...
class TestSavePriceTrigger:
    def test_create_file_and_add(self, tmp_path: Path) -> None:
        ...
        save_price_trigger(
            path, PriceTrigger("KRX:005930", buy_at=krw("70000"), sell_at=krw("90000"))
        )
        plan = YamlPriceTriggerLoader(path).load()
        t = plan.trigger_for("KRX:005930")
        assert t is not None
        assert t.buy_at == krw("70000")
        assert t.sell_at == krw("90000")

    def test_upsert_replaces_existing(self, tmp_path: Path) -> None:
        path = tmp_path / "triggers.yaml"
        save_price_trigger(path, PriceTrigger("KRX:005930", buy_at=krw("70000"), sell_at=None))
        save_price_trigger(
            path, PriceTrigger("KRX:005930", buy_at=krw("65000"), sell_at=krw("88000"))
        )
        plan = YamlPriceTriggerLoader(path).load()
        assert len([t for t in plan.triggers if t.asset_id == "KRX:005930"]) == 1
        t = plan.trigger_for("KRX:005930")
        assert t is not None and t.buy_at == krw("65000") and t.sell_at == krw("88000")

    def test_keeps_other_triggers(self, tmp_path: Path) -> None:
        path = tmp_path / "triggers.yaml"
        save_price_trigger(path, PriceTrigger("KRX:005930", buy_at=krw("70000"), sell_at=None))
        save_price_trigger(
            path, PriceTrigger("NASDAQ:AAPL", buy_at=Money.of("180", Currency.USD), sell_at=None)
        )
        plan = YamlPriceTriggerLoader(path).load()
